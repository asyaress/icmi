...
@section('title', $gallery->translated('title').' - Galeri ICMI Kaltim')
@section('meta_title', $gallery->translated('seo_title') ?: $gallery->translated('title').' - Galeri ICMI Kaltim')
@section('meta_description', $gallery->translated('seo_description') ?: \Illuminate\Support\Str::limit((string) $gallery->translated('description'), 160))
        ...
        <div class="row">
            <div class="col-lg-8">
                <article class="binduz-er-blog-details-box">
                    ...
                    <h1 class="binduz-er-title mb-3">{{ $gallery->translated('title') }}</h1>
                    @if($gallery->translated('description'))
                        <p class="mb-4">{{ $gallery->translated('description') }}</p>
                    @endif
                        ...
                        @forelse($gallery->items as $item)
                            <div class="col-md-6">
                                <div class="border rounded p-2 h-100 bg-white">
                                    <img src="{{ asset('storage/'.$item->image_path) }}" alt="{{ $item->caption ?? $gallery->translated('title') }}" class="img-fluid rounded mb-2">
                                    @if($item->caption)
                        ...
                        @forelse($relatedGalleries as $related)
                            <div class="binduz-er-sidebar-latest-post-item">
                                <div class="binduz-er-thumb">
                                    <img src="{{ $related->cover_image ? asset('storage/'.$related->cover_image) : asset('assets/images/latest-post-1.jpg') }}" alt="{{ $related->translated('title') }}">
                                </div>
                                <div class="binduz-er-content">
                                    <span><i class="fal fa-calendar-alt"></i> {{ optional($related->published_at)->format('d M Y') }}</span>
                                    <h4 class="binduz-er-title"><a href="{{ route('galeri.show', $related->slug) }}">{{ $related->translated('title') }}</a></h4>
                                </div>
                            </div>
                        @empty
